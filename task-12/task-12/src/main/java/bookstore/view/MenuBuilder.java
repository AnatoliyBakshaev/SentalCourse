...
public class MenuBuilder implements IBuilder {
    ...
    public void buildMenu() {
        ...
        IMenu settingsMenu = createSettingsMenu();  // ← НОВОЕ
        ...
        mainMenu.addMenuItem(new MenuItem(
                "настройки",
                () -> System.out.println("переход в меню настроек..."),
                settingsMenu
        ));
        ...
        addBackToSubMenu(settingsMenu);
    }


    private IMenu createSettingsMenu() {
        IMenu menu = new Menu("настройки");
        ...
        menu.addMenuItem(new MenuItem(
                "изменить период залежавшихся книг",
                () -> controller.changeOldMonths()
        ));

        menu.addMenuItem(new MenuItem(
                "включить/отключить авто-выполнение запросов",
                () -> controller.changeAutoFulfill()
        ));
